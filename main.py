"""
History-SchemaKG Framework - Main Orchestrator
================================================
Entry point for the four-phase pipeline that builds a Knowledge Graph from History text.
...
from pipeline.phase_3_schema_induction import (
    dynamically_induce_concepts,
    ground_concepts_to_ontology,
)
from pipeline.phase_4_kg_construction import (
    build_knowledge_graph,
    export_graph_to_neo4j_csv,
)
...
# =============================================================================
# Set to True if you want to skip Phase 1 & 2, using the saved file Phase2_Response.pkl from Phase 2.
...
def main():
    ...
    print(
        f"  - Resume Mode: {'ON (Skipping P1/P2)' if RESUME_FROM_PHASE_3 else 'OFF (Running full pipeline)'}"
    )
    print()
    ...
    if RESUME_FROM_PHASE_3:
        ...
        try:
            ...
            print(
                f"  - Entity-Entity (E-E): {sum(1 for t in all_triples if t['type'] == 'E-E')}"
            )
            print(
                f"  - Entity-Event (E-Ev): {sum(1 for t in all_triples if t['type'] == 'E-Ev')}"
            )
            print(
                f"  - Event-Event (Ev-Ev): {sum(1 for t in all_triples if t['type'] == 'Ev-Ev')}"
            )
            print(f"  - Unique Nodes: {len(unique_nodes)}")
            print()
            ...
            checkpoint_data = {"all_triples": all_triples, "unique_nodes": unique_nodes}
    ...
    try:
        induced_concepts = dynamically_induce_concepts(
            unique_nodes, all_triples=all_triples, use_real_llm=use_real_llm
        )
        print(
            f"✓ Part 3a Complete. Induced concepts for {len(induced_concepts)} nodes."
        )
        print()
    except Exception as e:
    ...
    print("Status: Part 3b - Grounding concepts to History ontologies...")
    try:
        grounded_nodes = ground_concepts_to_ontology(induced_concepts)
        print(
            f"✓ Part 3b Complete. Grounded {len(grounded_nodes)} nodes to ontology IDs."
        )
        print()
    ...
    print_pipeline_summary(
        text_segments if not RESUME_FROM_PHASE_3 else [],
        all_triples,
        grounded_nodes,
        knowledge_graph,
    )
    ...
    print("\n" + "=" * 80)
    print("Thank you for using History-SchemaKG Framework!")
    print("=" * 80)
