"""
Document Chunker for Phase 0 Transform & Phase 1 Ingestion
======================================================
Handles the loading of parsed Markdown files &  Plain txt,
segments them into context-aware chunks.
"""
...
class MarkdownChunker:
    ...
    ):  # Minimum chunk size before flushing
            ...
            if header_match:
                ...
                if current_text_buffer:
                    ...
                    if section_text:
                        ...
                        self.accumulated_sections.append(
                            (list(current_headers), section_text)
                        )
                ...
                if (
                    is_major_boundary and self.accumulated_length >= self.min_chunk_size
                ) or (self.accumulated_length >= self.max_chunk_size * 0.9):
    ...
    def _flush_accumulated_sections(self, chunks: List[str]):
        ...
        if len(full_text) <= self.max_chunk_size:
            ...
            for headers, text in self.accumulated_sections:
                ...
                if (
                    current_length + len(section_with_title) > self.max_chunk_size
                    and current_sub_sections
                ):
                    # Flush current batch
...
class PlainTextChunker:
    ...
    def _parse_plaintext(self, content: str) -> List[str]:
        chunks = []        
        lines = []
